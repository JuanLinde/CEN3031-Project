...
def read_all_data_from_test_table(db: Session):
    return db.query(Db_Test).all()

def read_data_from_test_table_by_id(db: Session, id: int):
    return db.query(Db_Test).filter(Db_Test.test_id == id).first()